...
def construct_sparse_cell_linked_list(pos: np.ndarray, linking_length: float) -> tuple[np.ndarray, ...]:
    ...
    grid_extent = (pos_max - pos_min) + 2.0 * linking_length # this is floor divided; add two linking lengths (top/bottom) to encapsulate all particles
...
def link_particles(
    ...
    for i in range(n_particles): # NOTE: see velocity dispersion function for more comments (duplicated logic)
        ...
        for j in range(cell_offsets[self_k], cell_offsets[self_k + 1]):
            ...
            _check_link(positions=positions, velocities=velocities, sigmas=sigmas, 
                    parents=parents, rank=rank, i=i, j=j,
                    linking_length_sq=linking_length_sq, velocity_factor=velocity_factor)
        ...
        for dx in range(-1, 2): # by construction, each particle's neighbours are within the nearest 3^3 cells
            for dy in range(-1, 2): 
                for dz in range(-1, 2):
                    ...
                    k = np.searchsorted(unique_cells, neighbour_cell_idx) # insertion index lets us check whether cell is empty

                    if k < len(unique_cells) and unique_cells[k] == neighbour_cell_idx: # ^ as then unique cell id = insertion index
                        ...
                        for j in range(cell_offsets[k], cell_offsets[k + 1]):
                            ...
                            pos_dx = positions[i,0] - positions[j,0]  # NOTE: np.linalg.norm will allocate temporary arrays here, keep it manual
                            pos_dy = positions[i,1] - positions[j,1]
                            pos_dz = positions[i,2] - positions[j,2]
