...
HI_mask = HI_mass > 1e8
gal_stellar_mass = cat.galaxies.get_dataset(name="mass_star", mask=HI_mask)

# get field halo indices of these galaxies
raw_field_halo_idx = cat.galaxies.get_membership(
    name="field_halo_index"
)  # the get_membership method can return variable-length arrays so does not allow masking
field_halo_idx = raw_field_halo_idx[HI_mask]  # Imask after loading
...
# I'm forming my analysis and want to grab some cosmological info from the catalogue
print(cat.sim_info.keys())  # let's see what's available

Hz = cat.sim_info("Hz")
print(cat.sim_info.description("Hz"))
boxsize = cat.sim_info("boxsize", to_units="Mpc", physical=True)
scale_factor = cat.sim_info("scale_factor")
scale_factor = cat.scale_factor  # this one is also on the catalogue object for convenience
...
for i, gal_idx in enumerate(idx_of_interest):
    snap_idx = cat.galaxies.get_particle_indices(
...
all_indices, all_offsets = cat.galaxies.get_membership("gas_indices"), cat.galaxies.get_membership("gas_offsets")
